...
def check_zipfile(check_path):
    ...
    if os.path.isfile(check_path):
        ...
        zip_file_path_lists = [check_path +
                               zipfff_path for zipfff_path in zip_file_list]

        # error_zipfile=check_zipfile(zip_file_path_lists)
        pool = mp.Pool()
        error_zipfile_temp = pool.map(
            check_zipfile_for_pool, zip_file_path_lists)
        error_zipfile = [
            error_zipfile_name for error_zipfile_name in error_zipfile_temp if error_zipfile_name != 0]
    else:
...
def check_and_download(error_zipfile_url_1, d="download"):
    ...
    if error_zipfile_url_1 != []:
        downloaded_files = [f for f in os.listdir(
            check_path) if f.split(".")[-1] == "txt"]
        if len(downloaded_files) == 1:
            year = downloaded_files[0].split(".")[0]
        elif len(downloaded_files) > 1:
            target = [files.split(".")[0] for files in downloaded_files if int(
                files.split(".")[0]) > 2002]
            if len(target) == 1:
                year = target
            else:
        ...
        all_missing_data_urls = [(u, check_path + "/")
                                 for u in all_urls_t if u.split("/")[-1] in error_zipfile_url_1]
...
if __name__ == "__main__":
    ...
    check_path = raw_input(
        "please enter the path of zipfiles or folders to check:")
    # check_path=__file__
    error_zipfiles = check_zipfile(check_path)
    error_zipfile_url_1 = [error_zipfile.split(
        '/')[-1] for error_zipfile in error_zipfiles]
