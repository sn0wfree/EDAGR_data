...
def check_Missing_file(target_url_a, target_path, download=False):
    ...
    while 1:
        ...
        downloaded_files = [
            f for f in downloaded_all_files if f.split(".")[-1] == "zip"]
        if set(target_url_file_and_url.keys()) != set(downloaded_files):
            undownload_file = set(
                target_url_file_and_url.keys()) - set(downloaded_files)
            undownload_files = [(target_url_file_and_url[ffs], target_path)
                                for ffs in undownload_file]
            print "\n !!! Found %d missing file(s)"
            if download == False:
                download_input = raw_input(
                    "Need I auto-download the missing file(s)?[yes or no]")
...
def compare_and_unzip(target_year, download=False):
    ...
    # compare
    target_url_a, target_path, unzip_pardir = download.import_data(
        target_year, pardir_status=1)

    downloaded_files = check_Missing_file(
        target_url_a, target_path, download=False)
    # print target_path

    # return full path
    downloaded_files_full_path = [
        target_path + temp for temp in downloaded_files]

    target_code = [(tempss, unzip_pardir)
                   for tempss in downloaded_files_full_path]
    pool = mp.Pool()
