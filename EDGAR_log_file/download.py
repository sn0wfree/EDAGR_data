...
def python_download(url, target_path, symbol="UI-friendly", reporthook=None):
    ...
    if target_path == "default":
        (url_default, path_default, file_name) = (
            url, "logfile", "log20160330.zip")
    ...
    if symbol == "UI-friendly":
        ...
        sys.stdout.write("\rDownloading %s completed,Remaining %d(%0.2f%%) " % (url.split(
            "/")[-1], (total_count - download_count), float(download_count) * 100 / total_count))
        sys.stdout.flush()
    else:
        pass
